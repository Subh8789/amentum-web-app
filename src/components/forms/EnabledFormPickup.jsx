...
const EnabledFormPickup = ({ formData }) => {
   ...
   // const [checkedDocuments, setCheckedDocuments] = useState([]);
    ...
    const handleSubmit = async (event) => {
        ...
        if (collectionOption.hasCollectionOption) {
        ...
        // Validation for In-proxy collection option selection
        if (!collectionOption.hasCollectionOption) {
            if (!representativeName.trim()) {
                validationErrors.push("When In-Proxy option is selected, representative name is required.");
            }
        }
        // Validation for In-proxy collection option selection for notes*
        if (!collectionOption.hasCollectionOption) {
            if (!additionalComments.trim()) {
                validationErrors.push("When In-Proxy option is selected, note field are required to be filled.");
            }
        }
                                ...
                                <div className="bg-white rounded shadow-sm">
                                    ...
                                    <div className="border-bottom px-4">
                                        <div className="d-flex" style={{ gap: "32px" }}>
                                            ...
                                            <div className="py-3 text-muted">MANAGE PICKUP</div>
                                        </div>
                                    </div>
...
export default EnabledFormPickup;
